...
package main // import "backblaze.upspin.io/cmd/upspin-setupstorage-b2cs"
...
It sets up Backblaze B2 storage for your Upspin installation. You may skip this step
...
Setupstorage-aws creates an Backblaze B2 bucket and updates the server configuration files in $where/$domain/ to use
the specified bucket.

Before running this command, you should ensure you have an B2 account and that
the B2 CLI command line tool works for you. For more information, visit
...
func (s *state) clean(bucketName string) {
	...
	for listErr != io.EOF {
		...
		objs, c, listErr = bucket.ListObjects(context.Background(), 128, c)
		if listErr != nil && listErr != io.EOF {
			log.Printf("unable to get file listing from b2: %v", listErr)
		}
		for i := range objs {
			if err := objs[i].Delete(context.Background()); err != nil {
				log.Printf("unable to delete file from b2: %v", err)
			}
		}
	}
